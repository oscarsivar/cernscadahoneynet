/* $Id: decode.c,v 1.106.2.4.2.9 2005/09/16 21:06:35 roesch Exp $ */
...
{
    /* Land attack */
    if( p->ip_src.s_addr == p->ip_dst.s_addr )
    {
        if((runMode == MODE_IDS) && pv.decoder_flags.decode_alerts)
        {
            SnortEventqAdd(GENERATOR_SNORT_DECODE, 
    ...
    if( (p->ip_src.s_addr & 0xff000000) == 0x7f000000  || (p->ip_dst.s_addr & 0xff000000 ) == 0x7f000000 )/* BE */
#else
    if( (p->ip_src.s_addr & 0xff) == 0x7f || (p->ip_dst.s_addr & 0xff ) == 0x7f ) /* LE */
#endif
    {
        if((runMode == MODE_IDS) && pv.decoder_flags.decode_alerts)
        {
