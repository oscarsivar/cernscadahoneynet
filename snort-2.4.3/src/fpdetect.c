/*
**  $Id: fpdetect.c,v 1.17.2.1.2.3 2005/04/05 15:49:25 jhewlett Exp $
**
**  fpdetect.c
**
...
static INLINE int fpAddSessionAlert(Packet *p, OTNX *otnx)
{
    ...
    if ( ssn->alert_count >= MAX_SESSION_ALERTS )
        return 0;

    ssn->alert_gid[ssn->alert_count] = otnx->otn->sigInfo.generator;
    ssn->alert_sid[ssn->alert_count] = otnx->otn->sigInfo.id;
    ssn->alert_count++;
...
static INLINE int fpSessionAlerted(Packet *p, OTNX *otnx)
{
    ...
    for ( i = 0; i < ssn->alert_count; i++ )
    {
        ...
        if ( (p->packet_flags & PKT_REBUILT_STREAM)
                && ssn->alert_gid[i] == si->generator && ssn->alert_sid[i] == si->id )
        {
            return 1;
        }
    }
