/* $Id: snort.c,v 1.210.2.4.2.17 2005/09/16 21:06:35 roesch Exp $ */
/*
...
int SnortMain(int argc, char *argv[])
{
    ...
    */
    ...
    if(!pv.readmode_flag && !pv.test_mode_flag)
    {
...
int ParseCmdLine(int argc, char *argv[])
{
    ...
    */
...
#ifdef GIDS
#ifndef IPFW
    valid_options = "?A:bB:c:CdDefF:g:G:h:i:Ik:K:l:L:m:n:NoOpP:qQr:R:sS:t:Tu:UvVwXyzZ:";
#else
    valid_options = "?A:bB:c:CdDefF:g:G:h:i:IJ:k:K:l:L:m:n:NoOpP:qr:R:sS:t:Tu:UvVwXyzZ:";
#endif /* IPFW */
#else
    ...
    valid_options = "?A:bB:c:CdeEfF:G:h:i:Ik:K:l:L:n:NoOpP:qr:R:sS:TUvVwWXyzZ:";
#endif
    ...
    while((ch = getopt(argc, argv, valid_options)) != -1)
    {
        ...
        switch(ch)
        {
            ...
            case 'G':                /* snort loG identifier */
                ...
                {
                    pv.event_log_id = atoi(optarg);
                }
                if ((pv.event_log_id > 0xFFFF) || (pv.event_log_id == 0))
                {
            ...
            case 'z': /* set assurance mode (used with stream 4) */
                LogMessage("WARNING: The -z option will be deprecated in the "
                           "next release of snort.\n  Use the 'stateful' "
                           "configuration option in snort.conf\n");
...
#else
    if((pv.interface == NULL) && !pv.readmode_flag && !pv.test_mode_flag)
    {
            FatalError( "You must specify a network interface (-i), "
...
void *InterfaceThread(void *arg)
{
    ...
    if(pcap_loop(pd, pv.pkt_cnt, (pcap_handler) PcapProcessPacket, NULL) < 0)
    {
        if(pv.daemon_flag)
...
static void SigUsrHandler(int signal)
{
    ...
    else if ( signal == SIGNAL_SNORT_ROTATE_STATS )
    {
