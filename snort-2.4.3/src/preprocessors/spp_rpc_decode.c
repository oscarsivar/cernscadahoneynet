...
/* $Id: spp_rpc_decode.c,v 1.29.6.3 2005/09/14 19:09:10 amullican Exp $ */
/* spp_rpc_decode 
 * 
 * Purpose:
 *
...
void PreprocRpcDecode(Packet *p, void *context)
{
    ...
    if (!(p->preprocessors & PP_RPCDECODE))
    {
        DEBUG_WRAP(DebugMessage(DEBUG_RPC,"Packet isn't taged for RPC inspection\n"););
        return;
    }
