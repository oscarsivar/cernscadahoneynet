...
/* $Id: spp_portscan.c,v 1.46.8.2 2005/09/01 17:27:38 ssturges Exp $ */
...
void PortscanPreprocFunction(Packet * p, void *context)
{
    /*
    ...
    if(!(p->preprocessors & PP_PORTSCAN))
    {
        return;
    }
...
{
    LogMessage("WARNING: the portscan preprocessor will be deprecated in "
            "the next release of snort.  Please switch to using SFPortscan.\n");
...
void CreateServerList(u_char * servers)
{
    ...
    if(servers == NULL)
    {
        FatalError(MODNAME ": %s (%d)=> No arguments to portscan-ignorehosts preprocessor!\n", file_name, file_line);
    }
...
void PortscanIgnoreParseIP(char *addr, ServerNode* server)
{
    ...
    if(addr == NULL)
    {
        FatalError("%s(%d) => Undefined address in portscan-ignorehosts directive\n",
                   file_name, file_line);
    }
...
IpAddrSet *PortscanAllocAddrNode()
{
    ...
    if(homeAddr == NULL)
    {
        ...
        if(homeAddr == NULL)
        {
            FatalError("Unable to allocate space for portscan IP addr\n");
        }
    ...
    if(idx == NULL)
    {
        FatalError("Unable to allocate space for portscan IP address\n");
    }
