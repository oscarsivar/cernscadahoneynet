/*
** $Id: perf-base.c,v 1.7.2.2.2.5 2005/08/23 15:52:19 ssturges Exp $
**
** perf-base.c
**
...
int GetPktDropStats(SFBASE *sfBase, SFBASE_STATS *sfBaseStats)
{
    ...
    */
    ...
    else
    {
        if( sfBase->iReset == 0 )
        {
            if (pcapStats.ps_recv < sfBase->pkt_stats.pkts_recv)
            {
            ...
            if (pcapStats.ps_drop < sfBase->pkt_stats.pkts_drop)
            {
                /* Rollover of the pcap stat value */
                sfBaseStats->pkt_stats.pkts_drop = pcapStats.ps_drop +
                    (UINT32_MAX - sfBase->pkt_stats.pkts_drop);
            }
            else
            {
            sfBaseStats->pkt_stats.pkts_drop = pcapStats.ps_drop -
