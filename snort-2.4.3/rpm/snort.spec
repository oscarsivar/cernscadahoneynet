# $Id: snort.spec,v 1.1.2.8 2005/10/17 00:33:51 mnorton Exp $
...
if [ -d doc/signatures/signatures ]; then
	%__mv doc/signatures/signatures/* doc/signatures
    %__rm -d doc/signatures/signatures
fi
...
InstallSnort() {
   ...
   if [ "$1" = "plain" ]; then
	...
	%__mkdir_p -m 0755 $RPM_BUILD_ROOT%{_docdir}/%{realname}-%{version}/doc/signatures
...
#	%__install -p -m 0644 rules/*.rules $RPM_BUILD_ROOT/%{SnortRulesDir}
...
#	find doc/signatures -type f -maxdepth 1 -not -name 'Makefile*' -exec %__install -p -m 0644 {} $RPM_BUILD_ROOT%{_docdir}/%{realname}-%{version}/doc/signatures \;
