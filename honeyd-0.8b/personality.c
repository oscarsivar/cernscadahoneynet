...
#define TIME_CORRECT(x) do { \
	tmp.tv_sec = 0; \
	tmp.tv_usec = tmp.tv_usec % (x); \
	timersub(&tmpl->tv, &tmp, &tmpl->tv); \
} while (0)
...
tcp_personality_seq(struct template *tmpl, struct personality *person)
{
	...
	if (person->tstamphz) {
		...
		slowhz = tmp.tv_sec * tstamphz + tmp.tv_usec / ticks;
		TIME_CORRECT(ticks);
	...
	case SEQ_RI:
		...
		return (tmpl->seq);
	case SEQ_CLASS64K:
		tmpl->seq += (slowhz + 1) * 64000;
		return (tmpl->seq);
	case SEQ_I800:
		tmpl->seq += (slowhz + 1) * 800;
		return (tmpl->seq);
	case SEQ_RANDOM:
	default:
