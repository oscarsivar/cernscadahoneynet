...
#if IS_LINUX
static inline uint64 convert_dev_t(uint32 major, uint32 minor);
#endif        //Function used to extract a uint64 from _dev_t values (lstat).
...
void cUnixFSServices::Stat( const TSTRING& strName, cFSStatArgs &stat ) const throw(eFSServices)
{
	...
	#if IS_LINUX //linux has the makedev macro commented out and fails to convert between types
	stat.dev		= convert_dev_t(major(statbuf.st_dev), minor(statbuf.st_dev));
	//(uint64)((uint32)major(statbuf.st_dev) << _TWNBITSMAJOR | (uint32)minor(statbuf.st_dev));
	stat.rdev		= convert_dev_t(major(statbuf.st_rdev), minor(statbuf.st_rdev));
	//(uint64)((uint32)major(statbuf.st_rdev) << _TWNBITSMAJOR | (uint32)minor(statbuf.st_rdev));
	#else
...
#if IS_LINUX
//Function used to extract a uint64 from _dev_t values (lstat).
//Only required in Linux, where makedev is broken.
static inline uint64 convert_dev_t(uint32 major, uint32 minor)	
{
    return ((major << _TWNBITSMAJOR) | (minor));
}
#endif
